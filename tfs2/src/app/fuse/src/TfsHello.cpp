/**
 * @file TfsHello.cpp
 * @description FUSE: Filesystem in Userspace, mount tfs to local filesystem
 * a simple TFS file mount to local system
 * @author Chen Rishan
 * @date 2007-10-21
 */
...
int TfsHello::getattr (const char *path, struct stat *stbuf) {
        int res = 0;
        ...
        // Compare the private variable of the TfsHello object to the
...
int TfsHello::readdir(const char *path, void *buf, fuse_fill_dir_t filler,
...
int TfsHello::open(const char *path, struct fuse_file_info *fi) {
        string strPath (path);
...
int TfsHello::read(const char *path, char *buf, size_t size, off_t offset,
